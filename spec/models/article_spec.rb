...
RSpec.describe Article, type: :model do
  ...
  context "タイトルが75文字以上だった時" do
    it "記事が作成できない" do
     ...
     expect(article).to_not be_valid  # 記事が無効であることを期待
     expect(article.errors[:title]).to include("is too long (maximum is 75 characters)")  # エラーメッセージも確認
    end
  end
  ...
  context "本文が200文字以上だった場合" do
    it "記事が作成できない" do
       ...
       expect(article.errors[:body]).to include("is too long (maximum is 200 characters)")  # エラーメッセージも確認
    end
  end
end
